...
uint32_t write_flash(uint32_t * dst, char * src, uint32_t no_of_bytes)
{
  ...
  if( byte_ctr == FLASH_BUF_SIZE)
  {
    ...
    find_erase_prepare_sector(SystemCoreClock/1000, (uint32_t)flash_address);
    if(result_table[0] != CMD_SUCCESS)
    {
      return result_table[0];
    }

    write_data(SystemCoreClock/1000,(uint32_t)flash_address,(uint32_t *)flash_buf,FLASH_BUF_SIZE);
    if(result_table[0] != CMD_SUCCESS)
    {
      return result_table[0];
    }
...
void execute_user_code(void)
{
  ...
  user_code_entry = (void (*)(void))(*(uint32_t*)(USER_FLASH_START + 4));
